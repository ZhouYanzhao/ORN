...
void BNLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  height_ = height_ * nOrientation;

  // spatial
  spatial_num_ = height_ * width_;
  ...
  if (this->blobs_.size() > 0) {
    ...
    FillerParameter scale_param;
    if (this->layer_param_.bn_param().has_scale_filler()) {
      scale_param = this->layer_param_.bn_param().scale_filler();
    }
    else {
    ...
    FillerParameter shift_param;
    if (this->layer_param_.bn_param().has_shift_filler()) {
      shift_param = this->layer_param_.bn_param().shift_filler();
    }
    else {
     shift_param.set_type("constant");
     shift_param.set_value(0);
    }
    shared_ptr<Filler<Dtype> > shift_filler(GetFiller<Dtype>(shift_param));
...
void BNLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  caffe_cpu_gemv<Dtype>(CblasNoTrans, num_ * channels_, spatial_num_, Dtype(1. / (spatial_num_)), const_bottom_data,
  ...
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num_ * channels_, spatial_num_, 1, Dtype(-1),
  ...
  // statistic across spatial
  caffe_cpu_gemv<Dtype>(CblasNoTrans, num_ * channels_, spatial_num_, Dtype(1. / (spatial_num_)), buffer_blob_.cpu_data(),
    spatial_sum_multiplier_.cpu_data(), Dtype(0), spatial_statistic_.mutable_cpu_data());
  // statistic across batch
  caffe_cpu_gemv<Dtype>(CblasTrans, num_, channels_, Dtype(1. / num_), spatial_statistic_.cpu_data(),
    batch_sum_multiplier_.cpu_data(), Dtype(0), batch_statistic_.mutable_cpu_data());
...
void BNLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  // statistic across spatial
  caffe_cpu_gemv<Dtype>(CblasNoTrans, num_ * channels_, spatial_num_, Dtype(1), buffer_blob_.cpu_data(),
    spatial_sum_multiplier_.cpu_data(), Dtype(0), spatial_statistic_.mutable_cpu_data());
  // statistic across batch
  ...
  // statistic across spatial
  caffe_cpu_gemv<Dtype>(CblasNoTrans, num_ * channels_, spatial_num_, Dtype(1), const_top_diff,
    spatial_sum_multiplier_.cpu_data(), Dtype(0), spatial_statistic_.mutable_cpu_data());
  // statistic across batch
  caffe_cpu_gemv<Dtype>(CblasTrans, num_, channels_, Dtype(1), spatial_statistic_.cpu_data(),
  ...
  // statistic across spatial
  caffe_cpu_gemv<Dtype>(CblasNoTrans, num_ * channels_, spatial_num_, Dtype(1), const_bottom_diff,
    spatial_sum_multiplier_.cpu_data(), Dtype(0), spatial_statistic_.mutable_cpu_data());
  // statistic across batch
  caffe_cpu_gemv<Dtype>(CblasTrans, num_, channels_, Dtype(1), spatial_statistic_.cpu_data(),
    batch_sum_multiplier_.cpu_data(), Dtype(0), batch_statistic_.mutable_cpu_data());
  ...
  caffe_cpu_axpby(buffer_blob_.count(), Dtype(1), buffer_blob_.cpu_data(), Dtype(-1. / (num_ * spatial_num_)),
    bottom_diff);
